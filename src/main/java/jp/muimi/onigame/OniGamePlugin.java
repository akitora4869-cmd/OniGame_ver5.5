...
public final class OniGamePlugin extends JavaPlugin implements Listener, CommandExecutor, TabCompleter {
    ...
    @Override public boolean onCommand(CommandSender sender,Command command,String label,String[] a){
        ...
        if(a[0].equalsIgnoreCase("finalsky")&&sender instanceof Player p){
            ...
            if(a[1].equalsIgnoreCase("status")){msg(p,"Custom Sky: "+(getConfig().getBoolean("final-crimson.blood-sky.optifine-custom-sky",true)?"&aON":"&cOFF")+" &f/ test-time=&e"+getConfig().getLong("final-crimson.blood-sky.optifine-test-player-time",12000L)+" &f/ weather=&e"+"CLEAR");return true;}
    ...
    private void enforcePhaseTime(){
        ...
        else if(state==GameState.RUNNING)world.setTime(gameNightTime());
        // finalPhaseは各プレイヤーのsetPlayerTime()でCustom Sky専用時間を上書きする。
    }
    ...
    private void startFinalSkyTest(Player p){
        ...
        long playerTime=Math.floorMod(getConfig().getLong("final-crimson.blood-sky.optifine-test-player-time",12000L),24000L);
    ...
    private void clearFinalSkyTest(){clearFinalEffectTest();}
